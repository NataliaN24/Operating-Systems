...
echo "$KEY"   | egrep -q "^[a-zA-Z0-9_]+$" || { echo "Invalid key"; exit 1; }
echo "$VALUE" | egrep -q "^[a-zA-Z0-9_]+$" || { echo "Invalid value"; exit 1; }
TMP=$(mktemp)
FOUND=0

while IFS= read -r line; do
    # махаме whitespace отпред и отзад
    clean=$(echo "$line" | sed -E 's/^[[:space:]]+//; s/[[:space:]]+$//')
    
    # пропускаме празни редове и чисти коментари
    if [[ -z "$clean" || "$clean" =~ ^# ]]; then
        echo "$line" >> "$TMP"
        continue
    fi

    # ако редът започва с KEY=
    if [[ "$clean" =~ ^$KEY[[:space:]]*=(.*) ]]; then
        echo "#$line # edited at $DATE by $USER" >> "$TMP"
        echo "$KEY = $VALUE # added at $DATE by $USER" >> "$TMP"
        FOUND=1
    else
        echo "$line" >> "$TMP"
    fi
...
# ако ключът липсва, добавяме го в края
if [[ $FOUND -eq 0 ]]; then
    echo "$KEY = $VALUE # added at $DATE by $USER" >> "$TMP"
fi
