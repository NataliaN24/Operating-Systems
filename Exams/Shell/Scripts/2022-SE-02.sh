...
declare -A MIN_COUNT=( [0]=1 [1]=2 [2]=3 [3]=4 )

objects=$(find "$DIR" -mindepth 2 -type f -printf "%f\n" | awk -F '-' '{print $1 "-" $2}' | sort -u)

for object in $objects; do
    for class in 0 1 2 3; do
        files=($(find "$DIR/$class" -type f -name "${object}*" ! -L | sort))
        count=${#files[@]}
        min_req=${MIN_COUNT[$class]}
        if [ $count -gt $min_req ]; then
            toDelete=$(( count - min_req ))
            for ((i=0; i<toDelete; i++)); do
                rm -f "${files[i]}"
                usage=$(df "$DIR" | awk 'NR==2{gsub("%","",$5); print $5}')
...
find "$DIR" -type l ! -exec test -e {} \; -print | xargs -r rm -f
