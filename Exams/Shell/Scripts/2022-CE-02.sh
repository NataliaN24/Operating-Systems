...
STATUS=$(grep -E "^$DEVICE[[:space:]]" /proc/acpi/wakeup | awk '{print $3}')

if [[ -z "$STATUS" ]]; then
    echo "Device $DEVICE not found in /proc/acpi/wakeup"
    exit 1
fi

if [[ "$STATUS" == "*enabled" ]]; then
    echo "$DEVICE" > /proc/acpi/wakeup
    echo "Device $DEVICE disabled for wake-up"
else
    echo "Device $DEVICE is already disabled"
fi
