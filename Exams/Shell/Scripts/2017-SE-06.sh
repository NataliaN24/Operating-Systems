...
root_rss=$(ps -u 0 -o rss= | awk '{sum+=$1} END{print sum+0}')

awk -F: '{print $1, $3, $(NF-1)}' /etc/passwd | while read username uid homedir; do
    # Пропускаме root
    if [[ "$username" == "root" ]]; then
        continue
    fi

    # Проверка на условията за home директория
    broken_home=0
    if [[ ! -d "$homedir" ]]; then
        broken_home=1
    elif [[ "$(stat -c %U "$homedir")" != "$username" ]]; then
        broken_home=1
    elif [[ ! -w "$homedir" ]]; then
        broken_home=1
    fi

    if [[ $broken_home -eq 1 ]]; then
        # Намираме общото RSS на процесите на потребителя
        user_rss=$(ps -u "$username" -o rss= | awk '{sum+=$1} END{print sum+0}')

        if (( user_rss > root_rss )); then
            echo "Killing all processes of user $username (RSS: $user_rss KB > root RSS: $root_rss KB)"
            pkill -u "$username"
        fi
    fi
done
